...
class PenyuluhanHukum extends StatefulWidget {
  @override
  _PenyuluhanHukumState createState() => _PenyuluhanHukumState();
}
...
class _PenyuluhanHukumState extends State<PenyuluhanHukum> {
  ...
  TextEditingController _bentukMasalahController = TextEditingController();
  String? _filePath1;
  String? _filePath2;
  ...
  Future<void> _submitForm() async {
    final uri = Uri.parse('$ip/postPenyuluhan.php');
    final request = http.MultipartRequest('POST', uri);

    request.fields['nama'] = _namaController.text;
    request.fields['no_hp'] = _noHpController.text;
    request.fields['no_ktp'] = _noKtpController.text;
    request.fields['bentuk_permasalahan'] = _bentukMasalahController.text;

    if (_filePath1 != null) {
      request.files.add(await http.MultipartFile.fromPath('file_ktp', _filePath1!));
    }

    if (_filePath2 != null) {
      request.files.add(await http.MultipartFile.fromPath('file_laporan', _filePath2!));
    }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: <Widget>[
              ...
              TextField(
                controller: _bentukMasalahController,
                decoration: InputDecoration(
                  hintText: 'Masukkan Bentuk Masalah',
                  border: OutlineInputBorder(),
                ),
              ),
              SizedBox(height: 20.0),
